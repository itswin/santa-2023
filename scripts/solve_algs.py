...
parser.add_argument("--max_setup_moves", type=int, default=3)
...
if args.commutator_file:
    commutators = create_algs(args.commutator_file, 
        moves, move_map=move_map, max_wrong=args.max_wrong, announce_skip=False)
    ...
    if args.create_conjugates:
        ...
        conjugates = create_conjugates(commutators, conjugate_moves, max_setup_moves=args.max_setup_moves, max_wrong=args.max_wrong)
...
if num_difference <= wildcards:
    ...
    with open(f"data/algs_partial_sol.txt", "w") as f:
        f.write(solution)
